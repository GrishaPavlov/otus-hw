...
World::World(const std::string& worldFilePath) {
    ...
    if (stream.fail())
        std::cout << "Could not open file " << worldFilePath << std::endl;
    /**
...
void World::show(Painter& painter) const {
    ...
    // painter.draw(Point(0, 0), 1, Color(1, 0, 1));
...
void World::update(double time) {
     ...
     */
    // balls[0].position = (balls[0].position + Point(0., 1.) * std::sin(time) * 100);
    // std::cout << balls[1].position.y << std::endl;
